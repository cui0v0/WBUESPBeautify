...
// ==UserScript==
// @name         WBUESPPlus-ZongceEdition
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  WBU教务平台Plus-综测核分专用
// @author       Simprole
...
(function () {
  $(document).ready(function () {
    ...
    function setup_SimUI() {
      ...
      calc_btn.addEventListener("click", function () {
        if (!pickPanel.querySelector("svg.checked")) {
          ...
          var final_result = Math.round(calculate(filtered_subjects) * 100) / 100..toFixed(2);
          scorePanel.innerHTML = "<span>" + semesters_text + "</span><span>学期综测分数：</span><div class='score'>" + final_result + "</div><br><span>请提供下方文本框的内容以核对课程成绩：</span>";
          var subjects_info = [];
          filtered_subjects.forEach(function (s) {
            var single_subject = {
              "name": s.name,
              "score": s.score,
              "credit": s.credit
            };
            subjects_info.push(single_subject);
          });
          var final_json = {
            "grade": final_result,
            "subjects": subjects_info,
            "personal_info": document.querySelector("#Top1_divLoginName").innerText
          };
          var encrypt_content = Sim_Encrypt(JSON.stringify(final_json));
          var textarea = document.createElement("textarea");
          textarea.value = encrypt_content;
          textarea.readOnly = true;
          textarea.style.resize = "none";
          textarea.style.width = "25rem";
          textarea.style.height = "20rem";
          scorePanel.appendChild(textarea);
    ...
    var key = CryptoJS.enc.Utf8.parse(CryptoJS.MD5('WBUJQRGCZONGCE24').toString());

    function Sim_Encrypt(text) {
      var encrypted = CryptoJS.AES.encrypt(text, key, {
        iv: CryptoJS.enc.Utf8.parse("2024091420240914"),
