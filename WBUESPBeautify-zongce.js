...
(function(){
    ...
    if(document.location.href.includes("cjcx_list")){
        ...
        document.querySelectorAll("td a[style][href]").forEach((e)=>{
            ...
            if(subject_info.findIndex((s)=>s.id==e.parentElement.previousElementSibling.previousElementSibling.innerText)!=-1){
                let another_index = subject_info.findIndex((s)=>s.id==e.parentElement.previousElementSibling.previousElementSibling.innerText)
                let average = (scoreProcess(subject_info[another_index].score)+scoreProcess(e.innerText))/2;
                subject.score = e.innerText;
                subject_info[another_index].score = Math.round(average * 100) / 100..toFixed(2);
                simGrid.appendChild(create_grid(subject_info[another_index].title,subject.score,subject_info[another_index].time,subject_info[another_index].id,subject_info[another_index].credit,subject_info[another_index].type,guake_state,JsModData));
            }else{
                subject.score = e.innerText;
                ...
                simGrid.appendChild(create_grid(subject.title,subject.score,subject.time,subject.id,subject.credit,subject.type,guake_state,JsModData));
    ...
    function create_grid(title,score,time,id,credit,type,guake_state,JsModData){
        ...
        if(guake_state==true){
            top.querySelector(".score").style.color="red";
        }
    ...
    function setup_SimUI(){
        ...
        calc_btn.addEventListener("click",()=>{
            if(!pickPanel.querySelector("svg.checked")){
                ...
                let final_result = Math.round(calculate(filtered_subjects)* 100) / 100..toFixed(2);
                scorePanel.innerHTML = "<span>"+semesters_text+"</span><span>学期综测分数：</span><div class='score'>"+final_result+"</div><br><span>请提供文本框中的内容以核对课程成绩：</span>";
                let final_json = {
                    "grade":final_result,
                    "semesters":selected_semesters,
                    "personal_info":document.querySelector("#Top1_divLoginName").innerText,
                }
                const encrypt_content = Sim_Encrypt(JSON.stringify(final_json));
    ...
    const key = CryptoJS.enc.Utf8.parse(CryptoJS.MD5('WBUJQRGCZONGCE24').toString());
