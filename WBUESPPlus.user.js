...
(function(){
    ...
    $("body").append($(`<script>function JsMod(htmlurl,tmpWidth,tmpHeight){
        const inPage = SimpleIframe_Init();
    ...
    if(document.querySelector("pre")||document.querySelector("form[action='/jsxsd/xk/LoginToXk']#Form1")){
        ...
        const SimIframe = SimpleIframe_Init();
        SimIframe.classList.add("display_none");
        document.body.firstElementChild.before(SimIframe);
        SimIframe.src = window.location.origin + "/jsxsd/caslogin.jsp";
        SimIframe.addEventListener('load', function() {
            if (SimIframe.contentWindow && SimIframe.contentWindow.location.href) {
                if(SimIframe.contentWindow.location.href.includes("xsMain.jsp")){
                    if(recovery_attempts > 4) {
                        window.sessionStorage.removeItem("recovery_attempts");
                        window.location.replace(window.location.origin + "/jsxsd/caslogin.jsp");
                    }else{
                        window.sessionStorage.setItem("recovery_attempts", recovery_attempts + 1);
        ...
        if(document.documentURI.includes("xspj_edit.do?")){
            ...
            let pjBtn = document.createElement("input");
            ...
            pjBtn.addEventListener("click",()=>{autoClick(0)});
        ...
        if(document.location.href.includes("comeXkjglb")||document.location.href.includes("comeInXxxk")){
            if(document.body.clientHeight <= document.body.scrollHeight){
                let rate = document.body.clientHeight/document.body.scrollHeight;
        ...
        if(document.location.href.includes("xsxk_index")){
            ...
            document.querySelectorAll("iframe[frameborder][src]").forEach((e) => {
                e.style.height = "100%";
            });
        ...
        if(document.location.href.includes("cjcx_list")){
            ...
            filter_btn.addEventListener("click",()=>{
                ...
                document.querySelectorAll("#dataList tr:not(:last-child)").forEach((e)=>{
            ...
            filter_ui_btn.addEventListener("click",()=>{
        ...
        if(document.location.href.endsWith("kbxx_xzb") || document.location.href.endsWith("kbxx_teacher") || document.location.href.endsWith("kbxx_classroom") || document.location.href.endsWith("kbxx_kc")) selection_data = window.localStorage.getItem("selection_data") ? JSON.parse(window.localStorage.getItem("selection_data")) : {};
    ...
    if(document.querySelector("input.button[onclick='exitXk()']")){
        const exitBtn = document.querySelector("input.button[onclick='exitXk()']");
    ...
    if(document.querySelector("#kbtable")&&window.parent.document.querySelector("iframe#fcenter")){
        ...
        // let weeklyTopBar = special_div_create("weeklyTopBar","");
    ...
    function setup_SimUI(){
        ...
        SimUI.querySelector(".close_btn").addEventListener("click",async ()=>{
            SimUI.classList.add("hidden");
            mask.classList.add("hidden");
            await asyncSleep(295);
    ...
    function create_picker_option(option_class,text_class,text){
        ...
        option.querySelector("svg").addEventListener("click",(s)=>{
    ...
    function filter_function(checkbox,filter_by){
        ...
        if(!checkbox.classList.contains("checked")){
            document.querySelectorAll("."+filter_by).forEach((e)=>{
    ...
    function getSchedule(){
        let allSchedules = [];
        let tr = document.querySelectorAll("#kbtable tr");
        for(let i=2;i<tr.length;i++){
            const colSpan = tr[0].querySelector("th[colspan]").colSpan;
            let weeklySchedule = [];
            weeklySchedule.title = tr[i].firstElementChild.innerText;
            let dailySchedule = [];
            let dayTemp = 0;
            for(let j=1;j<tr[i].children.length;j++){
                let lesson_time = j - Math.floor(j/colSpan)*colSpan - 1;
                ...
                let lessons = [];
                tr[i].children[j].querySelectorAll(".kbcontent1").forEach((kbcontent)=>{
                    let lesson = {};
                    kbcontent.childNodes.forEach((cn)=>{
                        if(cn.nodeName=='#text'){
                            if(cn.nodeValue.includes("周")){
                                lesson.time = cn.nodeValue.slice(cn.nodeValue.indexOf("("));
                                let front_text = cn.nodeValue.slice(0,cn.nodeValue.indexOf("(")).replaceAll("\n","");
                    ...
                    if(tr[i].children[j].querySelectorAll(".kbcontent1").length>1){
                        lessons.push(lesson);
    ...
    if($(".kbcontent1")[0]){
        ...
        var hue_list = [];
        if(!$(".Nsb_layout_r")[0]){
            let classLists_eg = create_specific_element("div","","classLists");
            let classLists,tempJson;
            let tr = document.querySelectorAll("#kbtable tr");
            let th = tr[0].children;
            ...
            for(let i=1;i<=tr[1].children.length;i++){
                if(tr[1].children[i-1].innerHTML=='13'){
                    tr[1].children.length--;
                    for(let o=2;o<=tr.length;o++){
                        tr[o-1].children[i-1].remove();
                    }
                }
            }
            for(let i=1;i<=$(".kbcontent1").length;i++){
            ...
            let detail_desc = create_specific_element("div","<span class='header'>选中的课程节次</span><span class='class'>班级</span><span class='section'>星期 节次</span>","detail_desc","hidden");
            document.querySelector("body").appendChild(detail_desc);
            for(let i=3;i<=tr.length;i++){
                for(let j=2;j<=tr[i-1].children.length;j++){
                    let this_td = tr[i-1].children[j-1];
                    this_td.addEventListener("click",()=>{
                        document.querySelectorAll(".chosen").forEach(e=>{if(e!=this_td) e.classList.remove("chosen")});
            ...
            // if(document.location.href.includes("kbxx_xzb")){
            //     getSchedule();
            // }
        }else{
