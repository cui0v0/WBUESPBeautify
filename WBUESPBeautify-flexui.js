...
(function(){
    ...
    function leftMenuWidthOptimize(){
        ...
        if(leftMenu.clientWidth<110){
            leftMenu.classList.add("narrow_patch");
        }else{
            leftMenu.classList.remove("narrow_patch");
        }
    }
    if(document.querySelector("#LeftMenu1_divChildMenu")){
        ...
        // window.addEventListener("resize",leftMenuWidthOptimize);
    }
    //修复评教页面宽度不为100%
    ...
    function setupScalingPanel(){
        ...
        // $("#kbtable")[0].ontransitionend = (event) =>{
        //     if(event.target.style.transform){
        //         let e = event.target;
        //         if(e.style.transform!=""){
        //             let transform = e.style.transform;
        //             e.style.zoom = parseFloat(transform.slice(transform.indexOf("(")+1,transform.length-1));
        //             e.style.transform = "";
        //         }else{
        //             e.style.transform = parseFloat(e.style.zoom);
        //             e.style.zoom = "";
        //         }
        //     }
        // }
    ...
    function rateOptimize(){
        let leftWidth = document.querySelector("#LeftMenu1_divChildMenu").clientWidth;
        let rightWidth = document.querySelector("#kbtable").clientWidth;
        let rate = leftWidth/rightWidth;
        if(rate<=0.085){
            if(rate<=0.055){
                document.querySelector("#kbtable").style.zoom = 0.6;
            }else{
                document.querySelector("#kbtable").style.zoom = rate.toFixed(2)*10+0.05;
            }
        }
    }
    //fix a width
